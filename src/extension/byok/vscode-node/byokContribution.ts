...
import { LocalApiRelayAIBYOKLMProvider } from './localApiRelayAIProvider';
...
export class BYOKContrib extends Disposable implements IExtensionContribution {
	...
	private async _authChange(authService: IAuthenticationService, instantiationService: IInstantiationService) {
		if (authService.copilotToken && isBYOKEnabled(authService.copilotToken, this._capiClientService) && !this._byokProvidersRegistered) {
			...
			this._providers.set(ZAILMProvider.providerName.toLowerCase(), instantiationService.createInstance(ZAILMProvider, knownModels[ZAILMProvider.providerName], this._byokStorageService));
			...
			this._providers.set(LocalApiRelayAIBYOKLMProvider.providerName.toLowerCase(), instantiationService.createInstance(LocalApiRelayAIBYOKLMProvider, knownModels[LocalApiRelayAIBYOKLMProvider.providerName], this._byokStorageService));
