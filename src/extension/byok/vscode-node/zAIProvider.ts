...
export class ZAILMProvider implements BYOKModelProvider<LanguageModelChatInformation> {
	public static readonly providerName = 'zAI';
	...
	private _enableThinking(modelId: string): boolean {
		...
		return normalized.startsWith('GLM-4.6') ||
			normalized.startsWith('GLM-4.5') ||
			normalized.startsWith('GLM-4-Plus') ||
			normalized.startsWith('GLM-4.5-X') ||
			normalized.startsWith('GLM-4.5V') ||
			normalized.startsWith('GLM-4.5-Air') ||
			normalized.startsWith('GLM-4.5-AirX') ||
			normalized.startsWith('GLM-4.5-Flash') ||
			normalized.startsWith('	GLM-4-32B-0414-128K');
	}
	...
	private async getAllModels(apiKey: string): Promise<BYOKKnownModels> {
		if (!this._anthropicAPIClient) {
			this._anthropicAPIClient = new Anthropic({ apiKey, baseURL: "https://api.z.ai/api/anthropic" });
		}
		try {
			...
			this._logService.error(error, `Error fetching available ${ZAILMProvider.providerName} models`);
			throw new Error(error.message ? error.message : error);
		}
	}

	async updateAPIKey(): Promise<void> {
		this._apiKey = await promptForAPIKey(ZAILMProvider.providerName, await this._byokStorageService.getAPIKey(ZAILMProvider.providerName) !== undefined);
		if (this._apiKey) {
			await this._byokStorageService.storeAPIKey(ZAILMProvider.providerName, this._apiKey, BYOKAuthType.GlobalApiKey);
		}
	}
	...
	async updateAPIKeyViaCmd(envVarName: string, action: 'update' | 'remove' = 'update', modelId?: string): Promise<void> {
		if (action === 'remove') {
			...
			await this._byokStorageService.deleteAPIKey(ZAILMProvider.providerName, this.authType, modelId);
			this._logService.info(`BYOK: API key removed for provider ${ZAILMProvider.providerName}`);
			return;
		}
		...
		this._apiKey = apiKey;
		await this._byokStorageService.storeAPIKey(ZAILMProvider.providerName, apiKey, this.authType, modelId);
		this._logService.info(`BYOK: API key updated for provider ${ZAILMProvider.providerName} from environment variable ${envVarName}`);
	}
	...
	async provideLanguageModelChatInformation(options: { silent: boolean }, token: CancellationToken): Promise<LanguageModelChatInformation[]> {
		if (!this._apiKey) { // If we don't have the API key it might just be in storage, so we try to read it first
			this._apiKey = await this._byokStorageService.getAPIKey(ZAILMProvider.providerName);
		}
		try {
			if (this._apiKey) {
				return byokKnownModelsToAPIInfo(ZAILMProvider.providerName, await this.getAllModels(this._apiKey));
