...
class GLM46DefaultPrompt extends PromptElement<DefaultAgentPromptProps> {
	async render(state: void, sizing: PromptSizing) {
		const tools = detectToolCapabilities(this.props.availableTools);
...
class ZAIPromptResolver implements IAgentPrompt {
	static readonly familyPrefixes = ['GLM', 'zAI'];

	resolvePrompt(endpoint: IChatEndpoint): PromptConstructor | undefined {

		if (endpoint.model?.startsWith('GLM-4.6') ||
			endpoint.model?.startsWith('GLM-4.5')) {
			return GLM46DefaultPrompt;
		}
